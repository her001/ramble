...
// with OpenSSL (or a modified version of that library), containing parts
// covered by the terms of Apache License 1.0, the licensors of this Program
...
fn connect() {
	...
	match con {
		Result::Ok(_) => (),
		Result::Err(err) => panic!("Connection failed: {}", err),
	}
}
...
fn connect_and_join() {
	...
	match j_con {
		Result::Ok(_) => (),
		Result::Err(err) => panic!("Server join failed: {}", err),
	}
}
